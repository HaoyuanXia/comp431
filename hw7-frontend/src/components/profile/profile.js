...
import { postAvatar, updateProfile } from './profileActions'
...
const UpdateInfo = ({user, info, setInfo, dispatch}) => {
    return (
        <div className='updateInfo content'>
            <h3> Update Info </h3>
            <form className='form-horizontal' onSubmit={(event) => {
                    event.preventDefault();
                    validate(setInfo, updateProfile, dispatch)
                    }}>
                <div className='info'>
                    {info}
                </div>
                ...
                <div className='form-group'>
                    ...
                    <input type='number' className='form-control' id='phoneNum' placeholder='Phone Number' required='required'
                        ...
                        readOnly ref={(node) => userProfile.phoneNum = node} />
                    </div>
                </div>
                ...
                <div className='form-group'>
                    <input id='btn_update_profile' type='submit' className='btn btn-info' value='Update' />
                </div>
            </form>
        </div>
    )
...
const validate = (setInfo, updateProfile, dispatch) => {
...
export const Profile = ({user, info, setInfo, dispatch}) => {
    return(
        <div>
            <Nav user={user} />
                ...
                <div className='row'>
                    <div className='col-md-4 col-md-offset-1 profileImg'>
                        ...
                        <div className='row'>
                            ...
                            <input className='col-md-6'type='file' accept='image/*' onChange={(event) => {
                                avatar = event.target.files[0]
                                if(avatar != null) {
                                    const fd = new FormData()
                                    fd.append('image', avatar)
                                    dispatch(postAvatar(fd))
                                }
                            }}/>
                        </div>
                    </div>
            ...
            <div className='row'>
                ...
                <div className='col-md-6'>
                    <UpdateInfo user={user} info={info} setInfo={setInfo} dispatch={dispatch}/>
                </div>
            </div>
        </div>
    )
